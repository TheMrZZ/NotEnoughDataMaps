...
# We want a change that is greater than one. With the scale, it's 1/10000. 
# It's just to account for the lack of precision in Minecraft operations. 
...
execute unless score diff motion_x matches -1..1 run function motion_animator:creation/store_motion
execute if entity @s[tag=!motion_changed] unless score diff motion_y matches -1..1 run function motion_animator:creation/store_motion
execute if entity @s[tag=!motion_changed] unless score diff motion_z matches -1..1 run function motion_animator:creation/store_motion
